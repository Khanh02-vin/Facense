...
EXPLAIN_REPORT_PATH = Path("data/dataset_processed/explain_report.json")
VALIDATION_REPORT_PATH = Path("data/dataset_processed/validation_report.json")
EMBED_DIM = 768
...
def create_app() -> FastAPI:
    ...
    def root() -> dict:
        return {
            "name": "Facense",
            "version": app.version,
            "endpoints": ["/health", "/items", "/rank", "/encode", "/explain", "/validate"],
            "docs": "/docs",
        }
    ...
    @app.get("/explain")
    def explain() -> dict:
        """Return the Phase 4 explainability report.

        Generated by ``run_pipeline.py --phase-4``.
        """
        if not EXPLAIN_REPORT_PATH.exists():
            raise HTTPException(
                status_code=404,
                detail="No explain report found. Run: python src/run_pipeline.py --phase-4",
            )
        return json.loads(EXPLAIN_REPORT_PATH.read_text(encoding="utf-8"))

    @app.get("/validate")
    def validate() -> dict:
        """Return the Phase 5 validation report.

        Generated by ``run_pipeline.py --phase-5``.
        """
        if not VALIDATION_REPORT_PATH.exists():
            raise HTTPException(
                status_code=404,
                detail="No validation report found. Run: python src/run_pipeline.py --phase-5",
            )
